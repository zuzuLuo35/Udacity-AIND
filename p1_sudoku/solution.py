...
diagonal_units = [[rows[i] + cols[i] for i in range(len(rows))], [rows[::-1][i] + cols[i] for i in range(len(rows))]]
#[[rows[i] + cols[i] for i in range(9)], [rows[::-1][i] + cols[i] for i in range(9)]]
...
def naked_twins(values):
    ...
    twos = [box for box in values if len(values[box]) == 2]
    twins = [[twin1, twin2] for twin1 in twos for twin2 in peers[twin1] if values[twin1] == values[twin2]]
    for twin1, twin2 in twins:
      common_peers = peers[twin1] & peers[twin2]
